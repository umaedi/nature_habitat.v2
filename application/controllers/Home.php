...
class Home extends CI_Controller
{
	...
	public function index()
	{
		...
		$data['kategoriIndoor'] = $this->db->limit(4)->get_where('products', array('category' => 10));
		$data['kategoriOutdoor'] = $this->db->limit(4)->get_where('products', array('category' => 11));
		...
		$this->load->view('templates/navbar_nature', $data);
	...
	public function subscribe_email()
	{
		...
		if ($ada) {
			$this->session->set_flashdata('subscriber', "<script>
				swal({
				text: 'Oops.. The email you entered is already subscribed./',
				icon: 'warning'
				});
				</script>");
			redirect(base_url());
			...
			$this->session->set_flashdata('subscriber', "<script>
				swal({
				text: 'You have successfully subscribed. We will send notifications about new offers and products.',
				icon: 'success'
				});
				</script>");
			redirect(base_url());
