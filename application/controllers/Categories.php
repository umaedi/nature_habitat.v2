...
class Categories extends CI_Controller
{
	...
	public function category($c)
	{
		...
		if ($ob != NULL) {
			if ($ob == "latest") {
				$data['titleHead'] = 'Urutkan > Terbaru';
				$data['products'] = $this->Products_model->getAllProductsBySubCategory($cat, "");
			} else if ($ob == "az") {
				$data['titleHead'] = 'Urutkan > Abjad A-Z';
				$data['products'] = $this->Products_model->getAllProductsBySubCategory($cat, "az");
			} else if ($ob == "za") {
				$data['titleHead'] = 'Urutkan > Abjad Z-A';
				$data['products'] = $this->Products_model->getAllProductsBySubCategory($cat, "za");
			} else if ($ob == "pmin") {
				$data['titleHead'] = 'Urutkan > Harga Terendah';
				$data['products'] = $this->Products_model->getAllProductsBySubCategory($cat, "pricemax");
			} else if ($ob == "pmax") {
				$data['titleHead'] = 'Urutkan > Harga Tertinggi';
				$data['products'] = $this->Products_model->getAllProductsBySubCategory($cat, "pricemin");
			}
		} else if ($minprice != NULL || $maxprice != NULL) {
			if ($minprice == "") {
				$minprice = "0";
			...
			$data['products'] = $this->Products_model->getAllProductsBySubCategoryPrice($cat, $minprice, $maxprice);
		} else if ($promo != NULL && $promo == "true") {
			$data['titleHead'] = 'Penawaran > Promo';
			$data['products'] = $this->Products_model->getAllProductsBySubCategory($cat, "promo");
		} else if ($condition != NULL) {
			if ($condition == "1") {
				$data['titleHead'] = 'Kondisi > Baru';
				$data['products'] = $this->Products_model->getAllProductsBySubCategory($cat, "1");
			} else if ($condition == "2") {
				$data['titleHead'] = 'Kondisi > Bekas';
				$data['products'] = $this->Products_model->getAllProductsBySubCategory($cat, "2");
			}
		} else {
			$data['titleHead'] = '';
			$data['products'] = $this->Products_model->getAllProductsBySubCategory($cat, "");
		}
		$data['title'] = 'Kategori ' . $this->Categories_model->getNameSubCategoryBySlug($c) . ' - ' . $this->Settings_model->general()["app_name"];
		$data['css'] = 'products';
		$data['responsive'] = 'product-responsive';
		$data['slug'] = $c;
		$data['nameCat'] = $this->Categories_model->getNameSubCategoryBySlug($c);
		$data['cart'] = $this->order_model->getCartUser();
		$this->load->view('templates/header_nature', $data);
		$this->load->view('templates/navbar_nature');
		$this->load->view('page/categories', $data);
